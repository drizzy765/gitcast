...
from typing import Optional, List
from dotenv import dotenv_values, load_dotenv, set_key, unset_key
from ai.generator import generate_posts, generate_sprint_summary, refine_post, _ai_call
from ai.prompts import load_prompt_definitions, PROMPTS_FILE, article_prompt, article_refinement_prompt, get_prompt
from ai.formatter import split_into_thread
from ai.viral_patterns import get_all_patterns
from api.payload import validate_payload
...
async def safe_get_posts() -> list:
    from api.auth_middleware import LOCAL_USER_ID
    from core.cloud_client import cloud_get_posts

    local_posts = load_local_posts()
    cloud_posts = await cloud_get_posts(LOCAL_USER_ID)
    if cloud_posts:
        from storage.logger import _normalize_entry
        return [_normalize_entry(entry) for entry in cloud_posts]
    return local_posts
...
async def safe_save_post(post_data: dict) -> bool:
    ...
    new_entry = {
        ...
        "timestamp": post_data.get("timestamp") or datetime.now().isoformat(),
        "user_id": post_data.get("user_id", LOCAL_USER_ID),
        "session_id": post_data.get("session_id") or post_data.get("user_id", LOCAL_USER_ID),
        "posted_verified": post_data.get("posted_verified", False),
        "declined": post_data.get("declined", False) or post_data.get("posted_declined", False),
        "verified_at": post_data.get("verified_at", ""),
        "metrics_saved": post_data.get("metrics_saved", False),
    ...
    save_local_posts(posts)
    try:
        await cloud_save_post(new_entry)
    except Exception:
        pass
    return True
...
@router.get("/keys/status")
def get_ai_keys_status(user_id: str = Depends(get_current_user)):
    from config.settings import BYOK_KEY, BYOK_PROVIDER

    configured = bool(os.getenv("BYOK_KEY", BYOK_KEY))
    return {
        "groq": True,
        "byok": configured,
        "provider": os.getenv("BYOK_PROVIDER", BYOK_PROVIDER),
        "using_base_keys": not configured,
        "using_shared_key": not configured,
    }
...
def update_ai_key(request: Request, body: ProviderKeyUpdate, user_id: str = Depends(get_current_user)):
    ...
    try:
        set_key(str(_env_path()), "BYOK_KEY", key)
        set_key(str(_env_path()), "BYOK_PROVIDER", provider)
        os.environ["BYOK_KEY"] = key
        os.environ["BYOK_PROVIDER"] = provider
        _reload_key_runtime()
        stream_log("Keys", "OK", "BYOK key saved locally")
        track("api_key_added", {"provider": provider, "storage": "local"})
        return {"success": True, "provider": provider, "key_preview": mask_key(key)}
    except Exception as e:
        stream_log("Keys", "ERROR", f"{provider} key update failed: {e}")
...
def remove_ai_key(request: ProviderKeyRemove, user_id: str = Depends(get_current_user)):
    provider = "groq"
    try:
        unset_key(str(_env_path()), "BYOK_KEY")
        unset_key(str(_env_path()), "BYOK_PROVIDER")
        os.environ.pop("BYOK_KEY", None)
        os.environ.pop("BYOK_PROVIDER", None)
        _reload_key_runtime()
        stream_log("Keys", "OK", "BYOK key removed locally")
        return {"success": True}
    except Exception as e:
        stream_log("Keys", "ERROR", f"{provider} key removal failed: {e}")
        return {"success": False, "error": str(e)}
...
@router.post("/byok/save")
async def save_byok(request: Request):
    ...
    lines.append(f"BYOK_PROVIDER={provider}")
    env_file.write_text("\n".join(lines))
    ...
    return {"success": True, "message": "Key saved and active"}


@router.delete("/byok/remove")
async def remove_byok():
    from pathlib import Path

    env_file = Path.home() / ".gitcast" / ".env"
    if env_file.exists():
        lines = env_file.read_text().splitlines()
        lines = [
            line for line in lines
    ...
    os.environ.pop("BYOK_PROVIDER", None)
    _reload_key_runtime()
    return {"success": True, "message": "Reverted to shared key"}
...
@router.get("/diagnose")
async def diagnose_connectivity(user_id: str = Depends(get_current_user)):
    """Diagnoses cloud server connectivity."""
    ...
    if not results:
        results["cloud"] = {
            "configured": health.get("status") != "unreachable",
            "status": health.get("status", "unknown"),
            "error": "" if health.get("status") != "unreachable" else "Cloud server unreachable",
        }
